...
**Q: ログに「開始」と「完了」しか表示されず、写真が整理されません。**
> **A:** プログラムが「処理すべきファイルがない」と判断している可能性があります。以下の2点をご確認ください。
> * **フォルダの選択**: Googleフォトからダウンロードし、**解凍した直後のフォルダ**を選択していますか？（tarファイルまたはtgzファイルのままや、一度実行して画像が移動し、JSONだけが残ったフォルダでは動作しません）
> * **階層の確認**: 選択したフォルダの直下、あるいはサブフォルダに画像ファイルが存在するか確認してください。

**Q: 写真が「2026年（実行時の現在時刻）」のフォルダに振り分けられてしまいます。**
> **A:** 本ツールはデータの安全性を優先するため、**「出力先フォルダ」にすでにデータが存在する場合や、一度実行したフォルダを再利用した場合**、正しく日付を特定できず現在時刻（実行日）を割り当てることがあります。
> 正確に整理を行うため、実行の際は必ず**「新しく作成した空のフォルダ」を出力先に指定**してください。
## 📄 ログファイルの読み方
